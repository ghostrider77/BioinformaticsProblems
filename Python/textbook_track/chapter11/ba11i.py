...
def calc_probability_of_spectral_dictionary(spectrum, mass_table, threshold, max_score):
    amino_acid_masses = tuple(mass_table.values())
    nr_amino_acids = len(amino_acid_masses)
    m = len(spectrum)
    ...
    def solve(ix, jy):
        ...
        if ix <= 0 or jy < 0:
            return 0
        return sum(solve(ix-mass, jy-spectrum[ix-1]) for mass in amino_acid_masses) / nr_amino_acids
...
def main():
    ...
    result = calc_probability_of_spectral_dictionary(spectrum, mass_table, threshold, max_score)
    print(result)
