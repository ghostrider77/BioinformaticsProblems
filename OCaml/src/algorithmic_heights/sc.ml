...
let has_edge_from_source_to_next (graph : Graph.t) (source : int list) (next : int list) : bool =
    let node_has_neighbor_in_next node =
        let neighbors = Graph.get_neighbors graph node in
        List.exists (fun neighbor -> List.mem neighbor next) neighbors in
    List.exists node_has_neighbor_in_next source


let is_semi_connected (graph : Graph.t) : bool =
    ...
    match components with
        | ([] | [_]) -> true
        | _ :: cs -> Seq.for_all2 (has_edge_from_source_to_next graph) (List.to_seq components) (List.to_seq cs)


let () =
    let nr_examples = read_int () in
    let graphs = read_graphs nr_examples in
    let results = List.map is_semi_connected graphs in
