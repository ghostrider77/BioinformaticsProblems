...
object BA1J {
  ...
  private def calcFrequenciesForApproximateOccurrences(text: String, k: Int, d: Int): Array[Int] = {
    ...
    val kMerEncodings: Iterator[Int] = text.sliding(k).flatMap{ pattern =>
      ...
      val reverseNeighbours: Set[String] = neighbours.map(calcReverseComplement)
      (neighbours.iterator ++ reverseNeighbours.iterator).map(patternToNumber)
    }
  ...
  def mostFrequentApproximateKMersWithReverseComplements(text: String, k: Int, d: Int): List[String] = {
