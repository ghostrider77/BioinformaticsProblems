...
object BA5I {
  ...
  private def collectOverlapAlignment(backtrack: Array[Array[Byte]],
                                      string1: String,
                                      string2: String,
    ...
    loop(Nil, string1.length, maxColIx)
  }

  def calcOverlapAlignment(string1: String, string2: String): (Int, String, String) = {
    ...
    for {
      ...
      val deletion: Int = table(ix)(jy + 1) - 2
      val insertion: Int = table(ix + 1)(jy) - 2
      val matching: Int = table(ix)(jy) + (if (c1 == c2) 1 else -2)
    ...
    val (maxValue, maxColIx): (Int, Int) = table(n).zipWithIndex.maxBy{ case (score, _) => score }
    table(n)(m) = maxValue
    val (alignedString1, alignedString2): (String, String) =
      collectOverlapAlignment(backtrack, string1, string2, maxColIx)
  ...
  def main(args: Array[String]): Unit = {
    ...
    val (score, alignedString1, alignedString2): (Int, String, String) = calcOverlapAlignment(string1, string2)
    println(score)
